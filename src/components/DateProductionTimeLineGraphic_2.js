...
const DateProductionTimeLineGraphic_2 =({temporada})=>{
    const [dateList, setDateList] = useState(temporada.productionDates);
    const [priceList, setPriceList] = useState(temporada.temporadaProduccionPerdida);
    ...
    const CustomLine = ({ line }) => (
        <Path key="line" d={line} stroke="#FF0000" strokeWidth={apx(6)} fill="none" />
    );
...
export default DateProductionTimeLineGraphic_2